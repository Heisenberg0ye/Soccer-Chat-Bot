import requests
import click
from soccer_data_api.exceptions import APIErrorException
    ...
    def get_standings(self, league):
        ...
        try:
            req = self._get('competitions/{id}/standings'.format(
                        id=league_id))
            message = self.writer.standings(req.json(), league)
        ...
        if league:
            try:
                ...
                if len(fixtures_results["matches"]) == 0:
                    ...
                    message = self.writer.league_scores(fixtures_results,
                                              time, show_upcoming,
                                              use_12_hour_format)
            except APIErrorException:
